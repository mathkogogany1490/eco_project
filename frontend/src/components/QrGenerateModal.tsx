        ...
        try {
            ...
            const res = await fetch(
                `${process.env.NEXT_PUBLIC_API_BASE_URL}/auth/qr-generate/`,
                {
                    method: 'POST',
                    headers: {
                        ...(token && { Authorization: `Bearer ${token}` })  // 🔥 핵심
                    }
                }
            )
            ...
            if (!res.ok) {
                console.error('QR 생성 에러:', data)
                throw new Error(data.detail || 'QR 생성 실패')
            }
    
            const uploadUrl = `${origin}/mobile/upload?token=${data.token}`
            ...
            console.error(err)
            setError(err.message || 'QR 코드를 생성할 수 없습니다.')
